"""Unit tests for /api/plans/maintenance CRUD + complete action endpoints."""
...
def _install(app, monkeypatch):
    ...
    monkeypatch.setattr(plans_repo, "list_maintain_plans", fake_list)
...
def test_list_maintenance_plans_requires_auth(monkeypatch):
    _env(monkeypatch)
    app = create_app()
    r = fakeredis.aioredis.FakeRedis()
    app.dependency_overrides[get_redis] = lambda: r
    assert TestClient(app).get("/api/plans/maintenance").status_code == 401


def test_list_maintenance_plans_empty(monkeypatch):
    _env(monkeypatch)
    app = create_app()
    _install(app, monkeypatch)
    resp = TestClient(app).get(
        "/api/plans/maintenance",
...
def test_list_maintenance_plans_with_dev_filter(monkeypatch):
    _env(monkeypatch)
    app = create_app()
    _install(app, monkeypatch)
    resp = TestClient(app).get(
        "/api/plans/maintenance?dev_number=60270012",
        headers={"Authorization": f"Bearer {_tok()}"},
    )
    assert resp.status_code == 200
    assert resp.json()["data"] == {"items": []}
...
def test_create_maintenance_plan_requires_company_role(monkeypatch):
    _env(monkeypatch)
    app = create_app()
    _install(app, monkeypatch)
    ...
    resp = TestClient(app).post(
        ...
        json={
            ...
            "plan_name": "月度保养",
            "interval_days": 30,
            "next_due_at": "2026-05-01T08:00:00Z",
        },
...
def test_delete_maintenance_plan_not_found(monkeypatch):
    _env(monkeypatch)
    app = create_app()
    _install(app, monkeypatch)
    ...
    monkeypatch.setattr(plans_repo, "get_maintain_plan", fake_get)
    from ruisheng_api.api import plans as plansapi

    monkeypatch.setattr(plansapi, "apply_tenant_context", fake_apply)

    resp = TestClient(app).delete(
        "/api/plans/maintenance/999",
