...
test.describe('全功能页面巡检', () => {
  ...
  test('配置类页面的新增和保存入口可用', async ({ page }) => {
    ...
    await page.getByLabel('Point ID').fill('12')
    await page.getByLabel('名称').fill('流量')
    await page.getByRole('button', { name: '保存' }).click()
    ...
    await page.getByLabel('点位 ID').fill('11')
    ...
    await page.getByLabel('计划名').fill('早班启动')
    await page.getByLabel('Cron 表达式').fill('0 8 * * *')
    await page.getByRole('button', { name: '保存' }).click()
