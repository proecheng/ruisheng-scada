...
- 2026-08-23 rollout correction 2：`.3` 在 Docker 前再次 fail-closed，暴露目标机只有 Windows PowerShell 5.1，且两份 Windows 校验器使用了 `New-Item` 不支持的 `-LiteralPath`；包内 Docker 29 归档解析本就依赖规格要求的 PowerShell 7.3+。已把所有目录创建改为 `New-Item -Path`，并在包外/包内校验器入口显式拒绝低于 7.3 的运行时；目标机仅允许安装经官方 Release SHA-256 与 Microsoft Authenticode 双重验证的 PowerShell 7.6.5 x64 MSI。`.2`/`.3` 保持不可变审计证据，现场改用 `.4`。KEEP：不降级归档解析、不绕过真实性门禁、不在 Windows PowerShell 5.1 下继续执行，且不改变 B-04 `2/BLOCKED` 终态。
- 2026-08-23 rollout correction 3：`.4` 在 Docker 前签名门禁 fail-closed，现场复现确认 Windows OpenSSH 9.5.6.1 的 `ssh-keygen -Y verify` 会在读取 .NET 匿名 stdin 管道时持续挂起，而同一签名从受保护文件重定向输入可在百毫秒内通过。已将包外/包内 Windows 校验器改为固定并验证系统 `cmd.exe`，拒绝路径空白与命令元字符，禁用 AutoRun/延迟展开后从受保护候选快照重定向 `SHA256SUMS`，继续保留 30 秒超时和 Docker 前失败语义；Windows Docker E2E 夹具同步迁移到受保护发布根，以实际执行原子发布门禁。`.2`/`.3`/`.4` 保持不可变审计证据，现场改用 `.5`。KEEP：不改变签名原始字节、principal、namespace、固定指纹、完整快照、系统工具 ACL 或 B-04 `2/BLOCKED` 终态。
- 2026-08-23 rollout correction 4：`.5` 已通过包外发布者签名和全包哈希，但包内校验器在 Docker 前 fail-closed；原因是其隔离 `DOCKER_CONFIG` 被创建在候选快照根内，随后严格候选 allowlist 正确将该运行时目录判为额外内容。已把 Docker 配置迁移到受保护工作根下的独立兄弟目录，并在统一 `finally` 中与候选快照分别清理；部署契约新增禁止运行时目录嵌入候选根的断言。`.2`/`.3`/`.4`/`.5` 保持不可变审计证据，现场改用 `.6`。KEEP：不放宽 19 文件 allowlist、不跳过包内纵深校验、不提前加载镜像，且不改变 B-04 `2/BLOCKED` 终态。
