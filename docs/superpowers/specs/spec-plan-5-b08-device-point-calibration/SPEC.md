...
- Qualification starts only from a protected package-external bootstrap; the v3 candidate contains the signed static toolchain but no executable launcher. Publisher modes are the closed set `ValidatorSchema`, `ValidatorProfile`, `ValidatorLegacy` and `Receipt`, with exact mode-specific parameters and no v2 fallback. Only `ValidatorProfile` invokes the Windows/POSIX protected publisher's OS-fixed freshness provider executable, config and trust-root path. It creates a one-time challenge, locks profile/policy/root/verifier snapshots, and must return `BLOCKED/INVALID` before validator startup if the provider is missing, fails or cannot be authenticated; the validator consumes the same witnessed root bytes. Public `validate` does not accept freshness parameters and lacks trusted publisher context, while the internal handoff verifies every binding against an attestation signed under the OS-fixed config/key.
- The toolchain archive is canonical single-member gzip plus deterministic strict USTAR with exact regular-file allowlist including `tools/trust_root_freshness.py`, zero padding and bounded zero trailer. General OCI processing is bounded before allocation by 4 MiB JSON/config, 32,768 outer members, 8 GiB per member, 32 GiB total expansion and 64 MiB aggregate Docker metadata; raw Docker outer and nested-layer tar headers reject every PAX/GNU extension before its payload can be allocated, and duplicate outer members are invalid. Migration inputs are at most 4,096 files, 2 MiB each and 64 MiB total, and OCI whiteouts must be zero-length regular files with valid non-dot targets.
...
- Trust-root provisioning is an external blocker even after offline validator completion. The v1 signed request binds site, one-time challenge, candidate logical identity, Profile ID/hash, payload, canonical gate, validator ID/source, verifier ID/tool hash, and root plus policy `(id, version, revocation_sequence, sha256)` state; the attestation additionally binds provider/witness key, trusted observation/expiry and monotonic state ID/counter. Exact state is idempotent; rollback, revocation rollback, same-version hash conflict, ID switch, replay, time or monotonic-counter failure is invalid, while unavailable provider or a local state ahead of the provisioned high-water is blocked. Qualification is read-only and never advances that high-water. A real authority outside the rollbackable repository/system disk must still provision both root and policy high-water states using TPM NV/equivalent monotonic hardware state or an independent remote witness, including approved selection, enrollment, witness key/identity and administrator file-replacement, whole-disk rollback, concurrency and failure evidence. Repository mocks, local ACLs and local state cannot close B-08-T.
