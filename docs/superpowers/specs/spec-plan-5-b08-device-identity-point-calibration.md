...
| Eligibility Schema and validator | `complete_offline` | v5 Schema/validator、canonical gate digest、四角色与 role/subject 门禁、分类/线路/runtime 证据验证、v3 qualification toolchain 和 `ReleaseVerificationReceipt` 离线实现及正反回归完成；该状态不代表外部信任根、Windows verifier 或现场证据已 provisioned |
| Trust-root anti-rollback provisioning | `blocked_external_provisioning` | 在仓库外受保护地维护至少 `(root_id, root_version, revocation_sequence, root_sha256)` 高水位；拒绝降级、同版本异 hash 和撤销序列回退，并用 TPM NV 或独立远端 freshness witness 覆盖管理员/整盘回滚威胁 |
...
Qualification archive 必须是确定性严格 USTAR：固定成员顺序和精确 allowlist、regular-file-only、canonical gzip 头 `1f8b08000000000002ff`、恰好一个 gzip member、确定性 USTAR header、零成员 padding、2 至 21 个尾部零 block 且无尾随数据。每个工具成员至多 64 MiB，内部 manifest/JSON 至多 4 MiB，压缩及展开预算由固定成员集合预计算；Python、Shell 和 PowerShell 验证器必须给出相同拒绝结果。一般候选/OCI 预算另固定为 4 MiB JSON/config、32,768 archive 成员、8 GiB 单成员、32 GiB 总展开和 64 MiB 聚合 Docker metadata；Docker 外层归档及嵌套 layer 必须在 `tarfile` 分配扩展 payload 前扫描原始 header，任何 PAX/GNU 扩展 header 均 fail-closed，且外层成员名不得重复。API migration 只接受 `/app/alembic.ini` 与直接位于 `/app/alembic/versions` 的 source-only `.py`，至多 4,096 个、单个 2 MiB、合计 64 MiB。OCI layer 全局至多 1,000,000 成员；whiteout 只有零长度 regular file 合法，空目标、`.`/`..`、链接、目录、设备、稀疏类型或非零 payload 均在改变 overlay 状态前拒绝。
...
外置信任根的真实性还必须具有仓库外 freshness 状态。provisioning 至少持久保存 `(root_id, root_version, revocation_sequence, root_sha256)` 高水位；允许完全相同 tuple/hash 的幂等复核，但必须拒绝较低版本、较低撤销序列、同一 identity/version 下不同 hash 或任何不能证明不旧于高水位的 root。管理员可替换本地文件、恢复系统盘或回滚整盘快照属于威胁模型，因此普通文件 ACL、nofollow 单读和本地磁盘状态都不足以关闭此 blocker；高水位必须由 TPM NV/等效硬件单调状态或独立远端 freshness witness 保护并留下可复核 provisioning 记录。
...
- canonical legacy JSON SHA-256 `7F90E12CA93A26CA7899BA3AE9E939FD5CC7A17CB2B1254A33C9F79F7DC121D9`；extractor source SHA-256 `BDD85C0447EEB860FE8B3373AC015511211DC7944724D4E98F7929F7F2AEDF77`；point-profile Schema SHA-256 `93A23C65E786192B8855896C0C2EEAA4C755F0A94E4F2865D7EC27BF269A244F`；validator v5 source SHA-256 `4DB30F0504AC4CA7E024E50DD45C02A5D7BE1C731B17B3D5BE076CEE7A083644`；release artifacts source SHA-256 `CDEB719F1ED9DF0C0736915F18A1F547548A7AAA4533BFC6B3C273C5B35D11B9`；receipt producer source SHA-256 `7003C9F3F037AF7DE0E1E72C862B3680D92EA210A4701965FA2441A1EA2D8354`；package-external bootstrap SHA-256 `ABE132265D48FF92EE48844352317A34EE5486432795EC2736A3E7894CB55005`。Schema checked-in bytes 与 `render_schema_document()` 逐字节一致，canonical JSON 与 extractor 输出逐字节同摘要；与上一账本的唯一内容差异是更新后的 extractor source digest。
...
- `ruff check .` 通过，`ruff format --check .` 为 `317 files already formatted`，`mypy .` 为 `167 source files` 无问题，`uv lock --check --offline` 通过；PowerShell 7、Windows PowerShell 5.1 对 3 个修改脚本解析通过，Git Bash 对 2 个修改脚本 `bash -n` 通过。
