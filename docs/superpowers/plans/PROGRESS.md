...
## 当前状态：Plan 0 **Stage D 进行中（D0+D1+D2 完成 3/8）**

**最后更新**：2026-04-16（D2 autogenerate 初始 schema 完成 + 26 表 upgrade/downgrade 幂等验证通过 + 两轮 review 全 APPROVED + 推送）
**工作分支**：`feature/plan-0-foundation`
**最近 commit**（worktree）：`9f2f102 feat(db): D2 initial schema migration — 26 tables (autogenerated + hand-verified)`
**最新 tag**：`plan-0-stage-c-complete`（Stage D 未打 tag）
**master 最新 commit**：`3e1ba92 docs(progress): session handoff after D1 — D2 prep + key environment lessons logged`（master 待更新）
**SHARED_SCHEMA_VERSION**：`20260415`
**测试状态**：321 passed + 8 skipped（D2 后仍 baseline，无回归）

**下一步**：D3 — RLS + DB 角色 (ruisheng_gw/ruisheng_api) + PL/pgSQL 触发器函数（`set_updated_at`、`enforce_scene_tenant_consistency`、`fill_scene_views_snapshot`）。手工 revision（autogenerate 不覆盖这些）。
...
## 🔖 会话交接点（2026-04-16，D2 完成后）

**当前位置**：Stage D 进行中，**D0 + D1 + D2 完成 (3/8)**，下一步 **D3**

**D2 完成情况**（commit `9f2f102`）：
- ✅ `alembic/versions/20260416_e05529ef4abb_initial_schema_26_tables.py`（1034 行）
- ✅ upgrade/downgrade/re-upgrade 幂等通过，`\dt` 核对 26 张表
- ✅ 321 passed + 8 skipped，无回归
- ✅ 两轮 review（spec compliance + code quality）均 APPROVED
- ✅ 命名规范 / PG 特化类型 / server_default / 范围控制全部 PASS

**D3 需要做的事（RLS + 角色 + 触发器函数，手工迁移）**：
1. docker stack 保持运行（D2 没关）。如果停了：`docker compose -f docker-compose.dev.yml up -d`
2. 生成空 revision：`uv run alembic revision -m "rls + roles + trigger functions"`
3. 手工填充 upgrade()（**spec §4.1 + §3.7 + §4.1.1**）：
   - **DB 角色**（spec §3.7）：
     - `CREATE ROLE ruisheng_gw LOGIN PASSWORD '...' BYPASSRLS`（网关，不走 RLS）
     - `CREATE ROLE ruisheng_api LOGIN PASSWORD '...'`（API，走 RLS；每次会话 `SET LOCAL`）
     - `GRANT` 基础 schema 权限
   - **PL/pgSQL 函数**（spec §4.1.1）：
     - `set_updated_at()` — BEFORE UPDATE 触发器 body（通用 updated_at 维护）
     - `enforce_scene_tenant_consistency()` — scene_pages/scene_views BEFORE INSERT/UPDATE 校验租户一致性（§3.8.18 + §4.1.1 (4)）
     - `fill_scene_views_snapshot()` — scene_views BEFORE INSERT 反查 users 填 company/department（§4.1.1 (5)）
   - **触发器**：每个带 updated_at 的表加 `CREATE TRIGGER trg_<table>_updated BEFORE UPDATE ... EXECUTE FUNCTION set_updated_at()`
   - **RLS**：每个租户表 `ALTER TABLE ... ENABLE ROW LEVEL SECURITY` + `CREATE POLICY tenant_isolation ...`
4. downgrade() 反向：DROP POLICY → DISABLE RLS → DROP TRIGGER → DROP FUNCTION → DROP ROLE
5. `uv run alembic upgrade head` → 核对 `\df`（函数列表）+ `\du`（角色列表）
6. 冒烟测试：以 ruisheng_api 角色 `SET LOCAL app.current_usr_group = 'xxx'` → SELECT 只看见本租户数据
7. downgrade base → 再 upgrade 幂等
8. commit + push

**D3 关键风险**：
- 角色密码别明文入 git（用 placeholder `<password>`，或 env 变量驱动）
- RLS policy 写对需要仔细对照 spec（`current_setting('app.current_usr_group')`）
- 触发器函数 DDL 里的中文字符串会被 configparser 坑（只在 .ini 里是 GBK 问题，.py 里 UTF-8 OK，但要注意 psql 客户端编码）

**参考**：spec §4.1 规约表、§4.1.1 通用设施、§3.7 RLS 架构、§3.8.17/.18 租户一致性
...
- Task 顺序：~~D0 环境校验~~ ✅ → ~~D1 baseline~~ ✅ → ~~D2 初始 schema~~ ✅ → **D3 RLS + roles + 触发器函数** ← 下一步 → D4 fillfactor/autovacuum → D5 hypertable + retention + compression → D6 测试集成 → D7 收尾 tag
